...
class TimeSpec extends WordSpec with Matchers {
  ...
  "Calling isIncreasing" should {
    "be true for an empty or one-element sequence" in {
      Time isIncreasing Vector.empty shouldBe true
      Time isIncreasing Vector(Time()) shouldBe true
    }
    "be false for a not strictly increasing sequence" in {
      Time isIncreasing Vector(Time(), Time()) shouldBe false
  //    Time isIncreasing Vector(Time(), Time(1), Time()) shouldBe false
    }
    "be true for a strictly increasing sequence" in {
      Time isIncreasing Vector(Time(1), Time(2)) shouldBe true
      Time isIncreasing Vector(Time(), Time(1), Time(2)) shouldBe true
    }
  }
